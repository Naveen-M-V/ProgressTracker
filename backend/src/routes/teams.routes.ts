...
router.get('/', (req: Request, res: Response) => {
  try {
    const teams = TeamService.getAllTeams(req.user!);
...
router.get('/:id', (req: Request, res: Response) => {
  try {
    ...
    const team = TeamService.getTeamById(teamId, req.user!);
...
router.post('/', requireAdmin, (req: Request, res: Response) => {
  try {
    ...
    const team = TeamService.createTeam({ name, description, lead_id, member_ids }, req.user!);
...
router.put('/:id', (req: Request, res: Response) => {
  try {
    ...
    const updated = TeamService.updateTeam(teamId, { name, description, lead_id }, req.user!);
...
router.delete('/:id', requireAdmin, (req: Request, res: Response) => {
  try {
    const teamId = parseInt(String(req.params.id), 10);
    TeamService.deleteTeam(teamId, req.user!);
...
router.get('/:id/members', (req: Request, res: Response) => {
  try {
    const teamId = parseInt(String(req.params.id), 10);
    const members = TeamService.getTeamMembers(teamId, req.user!);
...
router.post('/:id/members', (req: Request, res: Response) => {
  try {
    ...
    const member = TeamService.addMember(teamId, parseInt(user_id, 10), req.user!);
...
router.delete('/:id/members/:userId', (req: Request, res: Response) => {
  try {
    ...
    TeamService.removeMember(teamId, userId, req.user!);
